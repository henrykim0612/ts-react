...
import { useReducer, createContext, Dispatch, useMemo, useEffect } from 'react';
import { START_GAME, CLICK_MINE, FLAG_CELL, INCREMENT_TIMER, NORMALIZE_CELL, OPEN_CELL, QUESTION_CELL, ReducerActions } from '../action';
...
export const TableContext = createContext<Context>({ // Context interface 가 없으면 tableData 타입이 never 가 됨..
  tableData: [],
  halted: true,
  dispatch: () => {},
});
  ...
  switch (action.type) {
    ...
    case OPEN_CELL: {
      ...
      const checkAround = (row: number, cell: number) => {
        ...
        const count = around.filter((v) => ([CODE.MINE, CODE.FLAG_MINE, CODE.QUESTION_MINE] as Codes[]).includes(v)).length as Codes;
        if (count === 0) { // 주변칸 오픈
          if (row > -1) {
            const near = [];
            if (row - 1 > -1) {
